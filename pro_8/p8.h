...
EZ_DIFF_INTEGRATE::EZ_DIFF_INTEGRATE(char * in_fn)
{
	...
	o_f_8 << "FUNCTION IS PARSED AS: " << func_type << " ** " << exp << "	"<< arg << variable << endl;
...
EZ_DIFF_INTEGRATE::EZ_DIFF_BY_ME(float a , float b, int n)
{
	...
	if (strcmp(func_type,"sin") == 0)
	{
		...
		for (i=0; i<n; i++)
		{
			df[i]=( pow(sin(arg*x_new),exp)- pow(sin(arg*x_old),exp) ) / (fabs(x_new-x_old));
			x_new=x_new+((b-a)/n);
			x_old=x_old+((b-a)/n);
		}
	...
	else if (strcmp(func_type,"cos") == 0)
	{
		...
		for (i=0; i<n; i++)
		{
			df[i]=( pow(cos(arg*x_new),exp)- pow(cos(arg*x_old),exp) ) / (fabs(x_new-x_old));
			x_new=x_new+((b-a)/n);
			x_old=x_old+((b-a)/n);
		}
	}
	...
	for (i=0; i<n-1; i++)
	{
		...
		if ( (df[i]>0 && df[i+1]<0) || (df[i]<0 && df[i+1]>0) )
		{
			...
			else if ( (df[i+1]-df[i])<0)
			{
				max++;
			}
		}
	}
	o_f_8 << "THERE ARE " << min << " MINIMUMS AND " << max << " MAXIMUMS FOR THIS " << func_type << " FUNCTION " << endl;
...
EZ_DIFF_INTEGRATE::EZ_INTEG_BY_ME(float a , float b, int n)
{
	...
	if (strcmp(func_type,"sin") == 0)
	{
		...
		for (i=1; i<=n-1; i++)
		{
			mid_term+=fabs(pow(sin(arg*(a+i*h)),exp));
		}
		integral=(h/2)*( fabs(pow(sin(arg*a),exp)) + (2*mid_term) + fabs(pow(sin(arg*b),exp)) );
	}
	else if (strcmp(func_type,"cos") == 0)
	{
		...
		for (i=1; i<=n-1; i++)
		{
			mid_term+=fabs(pow(cos(arg*(a+i*h)),exp));
		}
		integral=(h/2)*( fabs(pow(cos(arg*a),exp)) + (2*mid_term) + fabs(pow((cos(arg*b)),exp)) );
	}
